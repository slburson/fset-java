...
/**
 * A functional set that relies on a comparison method.  It is implemented as a binary
 ...
 * supplied to the <code>FTreeSet</code> constructor.
 *
 ...
 * (linear) time if the other set involved is also a <code>FTreeSet</code> and
 ...
 * containing <code>FTreeSet</code>) takes O(n) (linear) time.
 *
 ...
 *
 * <p><code>FTreeSet</code> is declared as implementing <code>SortedSet</code>.
 * However, there are some subtle semantic differences which the user should be
 * aware of; <code>FTreeSet</code> does not perfectly fulfill the contract of
 * <code>SortedSet</code>.
 *
 * <p>First, unlike <code>SortedSet</code>, <code>FTreeSet</code> does not
 ...
 *
 * <p><code>FTreeSet</code> accepts the null element; for ordering purposes,
 * it precedes all other elements.
 *
 * <p>Although <code>FTreeSet</code> functions correctly in the presence of
    ...
    implements SortedSet<Elt>, Comparable<FTreeSet<Elt>>, java.io.Serializable
{
     ...
     */
    public static <Elt> FTreeSet<Elt> emptySet(Comparator<? super Elt> comp) {
	if (comp == null) return emptySet();		// mostly for our own use
	else return new FTreeSet<Elt>(comp);
    }

    /**
     * Constructs an empty <code>FTreeSet</code> that uses the natural ordering
     * of the elements.
     */
    public FTreeSet() {
	tree = null;
    ...
    /**
     * Constructs a <code>FTreeSet</code> containing only <code>elt</code>, and
    ...
    /**
     * Constructs an empty <code>FTreeSet</code> that uses the supplied
     * <code>Comparator</code> to compare elements.
     *
     * @param c the comparator
     */
    public FTreeSet(Comparator<? super Elt> c) {
	tree = null;
     ...
     */
    public FTreeSet(Elt elt, Comparator<? super Elt> c) {
	comp = (Comparator<Elt>)c;
	tree = with(null, elt);
    }

    /**
     * Constructs a <code>FTreeSet</code> containing the same elements as
    ...
    /**
     * Constructs a <code>FTreeSet</code> whose elements are the components of
     ...
     * <code>FTreeSet</code> and uses the same ordering as this set; otherwise it
     ...
     * <code>obj</code> is another <code>FTreeSet</code>, if the two sets do not
	...
	if (obj == this) return 0;
		 ...
		 !(obj instanceof FTreeSet && eql(comp, ((FTreeSet)obj).comp)))
	    throw new ClassCastException();
	else return compareTo(tree, ((FTreeSet)obj).tree);
    }
	...
	if (tree == null) return this;
		...
		compare(le, fromElement) < 0) return new FTreeSet<Elt>(comp);
    ...
    private FTreeSet(int x, Object _tree, Comparator<? super Elt> _comp) {
    ...
    /**
     * Saves the state of this <code>FTreeSet</code> to a stream.
     *
