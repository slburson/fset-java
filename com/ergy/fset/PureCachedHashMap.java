 ...
 * take O(<i>n</i> log <i>n</i>) time.  <code>union</code>, <code>restrictedFrom</code>,
 * and <code>restrictedTo</code> take O(n) (linear) time if the other map involved is
    ...
    implements Comparable, java.io.Serializable
{
    ...
    private static Subtree union (Subtree tree1, Subtree tree2) {
	...
	Subtree with (Object k, int khash, Object val) {
		...
		else return makeNode(equivUnion(key, value, k, val),
				     hash, val, left, right);
	    } else if (khash < hash) {
	...
	Subtree with (Object key, int khash, Object value) {
		...
		return new Leaf(insertObj2(keys_vals, idx, key, value),
				insertInt(hashes, idx, khash));
				  ...
				  ? equivUnion(keys_vals[idx], keys_vals[idx + nkeys],
					       key, value)
				  : key),
				 khash, value,
	...
	Subtree union (Subtree subtree2, int lo, int hi) {
		...
		return unionLeaves(keys_vals, hashes, leaf2.keys_vals, leaf2.hashes,
				   lo, hi);
	    } else {
		Node node2 = (Node)subtree2;
		int hash2 = node2.hash;
		Subtree new_left = union(trim(lo, hash2),	// i.e. `this.trim'
					 trim(node2.left, lo, hash2),
					 lo, hash2);
		Subtree new_right = union(trim(hash2, hi),
		...
		if (entry1 == null)
		    ...
		    Object k = equivUnion(entry1.key, entry1.value,
	...
	// makes a node.
	static Subtree unionLeaves (Object[] keys_vals1, int[] hashes1,
		...
		if (i1 == len1) {
			...
			if (k1 == null ? k2 == null : k1.equals(k2)) {
			    ...
			    keys_vals[nelts] = equivUnion(k1, keys_vals1[i1 + nkeys1],
